...
public class AddressServiceImpl implements AddressService{
    ...
    public ShippingAddrDto saveAddress(ShippingAddrDto addrDto, Integer customerId) {
        CustomerEntity customerEntity = customerRepository.findById(customerId).orElseThrow();// orElseThrow() ==>
        //if not used it will return Optional object
        // orElseThrow() ==> if record is available it will give object directly  else thr. exception(remaining below logic will not execute)
    ...
    public List<ShippingAddrDto> getCustomerAddresses(Integer customerId) {
        List<ShippingAddressEntity> addressEntityList = addressRepository
                .findByCustomerCustomerIdAndDeleteSw(customerId,"N");
    ...
    public boolean deleteAddress(Integer addrId) {
        ...
        //HARD DELETE - DELETE THE DATA FROM DB
...
//        }
        ...
        //SOFT DELETE - DATA WON'T BE DELETED FROM DB(but it will be inactive record)
        Optional<ShippingAddressEntity> byId = addressRepository.findById(addrId);
        if(byId.isPresent()){
            ShippingAddressEntity addressEntity = byId.get();
            addressEntity.setDeleteSw("Y");//delete switch ==> Y ==> inactive , N ==> active
